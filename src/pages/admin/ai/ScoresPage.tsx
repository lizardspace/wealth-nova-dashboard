...
export default function ScoresPage() {
  ...
  const alertClients = scores.filter(client => parseInt(client.evolution) <= -10);
            ...
            <div className="text-sm text-amber-800">
              {alertClients.map(client => (
                <div key={client.id} className="mb-1">
                  Le client <span className="font-medium">{client.client}</span> a perdu{' '}
                  <span className="font-medium">{client.evolution.replace('-', '')}</span> points de
                  score ce mois-ci.
      ...
      <Tabs defaultValue="all">
        <TabsList>
          ...
          <TabsTrigger value="declining">
            En baisse ({scores.filter(s => s.status === 'declining').length})
          </TabsTrigger>
          <TabsTrigger value="stable">
            Stables ({scores.filter(s => s.status === 'stable').length})
          </TabsTrigger>
          <TabsTrigger value="improving">
            En hausse ({scores.filter(s => s.status === 'improving').length})
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="declining" className="mt-4">
          <Card>
            ...
            <CardContent>
              <DataTable columns={columns} data={scores.filter(s => s.status === 'declining')} />
            </CardContent>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="stable" className="mt-4">
          <Card>
            ...
            <CardContent>
              <DataTable columns={columns} data={scores.filter(s => s.status === 'stable')} />
            </CardContent>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="improving" className="mt-4">
          <Card>
            ...
            <CardContent>
              <DataTable columns={columns} data={scores.filter(s => s.status === 'improving')} />
            </CardContent>
          </Card>
        </TabsContent>
      </Tabs>
