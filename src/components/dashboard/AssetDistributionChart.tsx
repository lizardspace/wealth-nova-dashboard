      ...
      <Card className="border-none shadow-md">
        ...
        <CardContent>
          <div className="h-[250px] w-full">
            {/* Augmenté la hauteur de 200px à 250px */}
            <ResponsiveContainer width="100%" height="100%">
              <PieChart margin={{ top: 0, right: 0, bottom: 0, left: 0 }}>
                {/* Ajouté margins explicites */}
                <Pie
                  data={data}
                  cx="50%"
                  cy="50%"
                  innerRadius={45}
                  outerRadius={80}
                  /* Ajusté pour mieux s'adapter à la hauteur augmentée */
                  paddingAngle={2}
                  dataKey="value"
                ...
                <Legend
                  ...
                  /* Ajusté la taille et le padding */
                />
              </PieChart>
            </ResponsiveContainer>
          </div>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>{selectedAsset?.value}% de votre patrimoine total</DialogDescription>
          </DialogHeader>
          <div className="grid gap-4">
            {selectedAsset?.details?.map((detail, index) => (
              <div
                key={index}
                className="bg-muted/20 p-3 rounded-md hover:bg-muted/30 transition-colors animate-fade-in"
                ...
                <div className="space-y-3 animate-fade-in">
                  ...
                  <ul className="text-xs space-y-2">
                    <li className="flex gap-2">
                      ...
                      <span>
                        Votre exposition à cette classe d'actifs{' '}
                        {selectedAsset?.value && selectedAsset.value > 30
                          ? 'est élevée'
                    ...
                    <li className="flex gap-2">
                      ...
                      <span>
                        ...
                        {selectedAsset?.name === 'Actions'
                          ? 'Bonne'
                          : selectedAsset?.name === 'Immobilier'
                            ? 'Excellente'
                            : 'Stable'}
                      </span>
                    </li>
                    <li className="flex gap-2">
                      ...
                      <span>
                        Diversification:{' '}
                        {selectedAsset?.details && selectedAsset.details.length > 2
                          ? 'Bonne'
                          : 'À améliorer'}
                      </span>
                    </li>
